      ...
      return `
        <div class="card" style="animation-delay:${index * 0.05}s" onclick="openModal(${item.id})">
          ...
          <div class="card-footer">
            <div class="steps-count">&#128203; ${item.steps.length} steps</div>
...
function openModal(id) {
  ...
  document.getElementById("mSteps").innerHTML = item.steps
    .map(
      (step, index) => `
        <div class="step" style="animation-delay:${index * 0.06}s">
          <div class="step-num" style="background:${item.colorDim};color:${item.color}">${index + 1}</div>
