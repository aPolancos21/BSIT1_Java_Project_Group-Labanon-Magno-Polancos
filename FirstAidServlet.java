 ...
 * FirstAidServlet
 *
 * GET /FirstAidServlet?search=burn    → returns filtered first aid guides as JSON
 * GET /FirstAidServlet?category=burns → returns guides by category
 * GET /FirstAidServlet                → returns all guides
 *
 * This servlet serves first aid data to the frontend.
 * The HTML file (index.html) works standalone too — this servlet
 * is used when running inside Apache Tomcat.
 */
@WebServlet("/FirstAidServlet")
public class FirstAidServlet extends HttpServlet {

    /**
     * All first aid guide data as a JSON string.
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // Allow frontend (same origin) to access this
        response.setHeader("Access-Control-Allow-Origin", "*");

        String search = request.getParameter("search");
        String category = request.getParameter("category");

        // NOTE: For full search/filter in Java, you would parse the JSON and filter.
        // For simplicity, we return all data and let the frontend handle filtering.
        // This is appropriate for a small dataset like this.
